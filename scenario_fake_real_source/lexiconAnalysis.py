...
import \
    seaborn as sns  # Python visualization library based on matplotlib provides a high-level interface for drawing attractive statistical graphics
...
def adjust_distribution(df, n_real, n_fake, seed):
    ...
    if n_fake > len(df_fake):
        print('Inconsistencia na quantidade de noticias falsas!', n_fake, len(df_fake))
    if n_real > len(df_real):
        print('Inconsistencia na quantidade de noticias reais!', n_real, len(df_real))
...
def save_results(list_results, filename):
    with open("output_values/" + filename + '.txt', 'a') as f:
        f.write(str(list_results[-1]) + "\n")
        # for item in list_results:
        #    f.write(str(item) + "\n")


def save_results_to_plot(list_results, filename):
...
def testrep_roc_ap_curve_metrics_LEX(clf, df_train_input, df_testset_input, rep, algo_name, plt_roc, plt_pr, filename):
    ...
    for i in range(rep):
        ...
        save_results(aucs, filename + "_" + algo_name + "_ROCAUC")
    ...
    std_ap = np.std(aps)
    lab = ' %s Mean PR-AUC = %.2f  $\pm$ %0.2f)' % (algo_name, avg_ap, std_ap)
    # plt.subplot(2, 1, 2)
    plt_pr.plot(recall, precision, label=lab, lw=2, alpha=alpha)

    save_results_to_plot(precision, filename + "_" + algo_name + "_precisionplot")
    save_results_to_plot(recall, filename + "_" + algo_name + "_recallplot")

    save_results_to_plot(mean_fpr, filename + "_" + algo_name + "_meanfprplot")
    save_results_to_plot(mean_tpr, filename + "_" + algo_name + "_meantprplot")
...
def testrep_roc_ap_curve_metrics_TFIDF(clf, df_train_input, df_testset_input, rep, algo_name, plt_roc, plt_pr,
                                       filename):
    tprs = []
    aucs = []
    y_real = []
    ...
    listFNR = []
    listFPR = []
    print("repeticoes para " + algo_name)
    for i in range(rep):
        start_time_tfidf = time.time()

        if not check_matrix_folder("tfidf_matrix_data/", 1500):
            df_train = adjust_distribution(df_train_input, zero_size_train, one_size_train, i)
            df_train = encode_label_to_roc(df_train)
            X_tfidf_train, y_tfidf_train, vectorizer = load_tfidf_values_for_test(df_train, None, None)
            clf.fit(X_tfidf_train, y_tfidf_train)
            pickle.dump(X_tfidf_train, open("tfidf_matrix_data/X_tfidf_train_" + str(i), "wb"))
            pickle.dump(y_tfidf_train, open("tfidf_matrix_data/y_tfidf_train_" + str(i), "wb"))
            pickle.dump(vectorizer, open("tfidf_matrix_data/vectorizer_" + str(i), "wb"))
            ...
            X_tfidf_train = pickle.load(open("tfidf_matrix_data/X_tfidf_train_" + str(i), "rb"))
            y_tfidf_train = pickle.load(open("tfidf_matrix_data/y_tfidf_train_" + str(i), "rb"))
            vectorizer = pickle.load(open("tfidf_matrix_data/vectorizer_" + str(i), "rb"))
            clf.fit(X_tfidf_train, y_tfidf_train)
            df_testset = adjust_distribution(df_testset_input, zero_size_test, one_size_test, i)
            df_testset = encode_label_to_roc(df_testset)
            X_test, Y_test = load_tfidf_values_for_testrep_with_vectorizer(df_testset, vectorizer)
        ...
        print("--- %s seconds per repetition ---" % (time.time() - start_time_tfidf))
        print("repetition", i)
        gc.collect()
...
def plot_all_roc_curves_test_repetitions(df_train_input, df_testset_input, repetitions, filename):
    ...
    clfs.append(('XGBClassifier_TFIDF', XGBClassifier(n_jobs=6)))
    ...
    clfs.append(('XGBClassifier_LEX', XGBClassifier(n_jobs=6)))
    ...
    for algo_name, clf in clfs:
        ...
        response = "%s\nMean AUC-PRC: %0.2f\nStd AUC-PRC: %0.2f\nMean Precision: %0.2f\nSTD Precision: %0.2f\n" \
                   "Mean Recall: %0.2f\nSTD Recall: %0.2f\nMean F1: %0.2f\nSTD F1: %0.2f\nMean TNR: %0.2f\nSTD TNR: %0.2f\n" \
                   "Mean TPR: %0.2f\nSTD TPR: %0.2f\nMean FNR: %0.2f\nSTD FNR: %0.2f\nMean FPR: %0.2f\nSTD FPR: %0.2f" \
                   % (algo_name, mean_ap, std_ap, mean_precision, std_precision, mean_recall, std_recall, mean_f1score,
                      std_f1score, \
...
def check_matrix_folder(directory, size):
    if len(os.listdir(directory)) == size:
        return True
    else:
        return False
...
def execute_test_scenarios(train_dataset_path, test_list_paths, repetitions, base_file_name):
    ...
    clean_directory("tfidf_matrix_data/")
...
train_dataset_path = '../../../data/scenario_fake_real_source/satire_pol/lexicon_rates_without_stopwords_REAL_POL-FAKE_ESTADAO_FOLHA_SENTENCES_ALL.csv'
test_list_paths = [
    '../../../data/scenario_fake_real_source/satire_pol/lexicon_rates_without_stopwords_REAL_ESP_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_pol/lexicon_rates_without_stopwords_REAL_ECO_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_pol/lexicon_rates_without_stopwords_REAL_CUL_ESTADAO_FOLHA_SENTENCES_ALL.csv'
]
repetitions = 500
...
train_dataset_path = '../../../data/scenario_fake_real_source/satire_esp/lexicon_rates_without_stopwords_REAL_ESP-FAKE_ESTADAO_FOLHA_SENTENCES_ALL.csv'
test_list_paths = [
    '../../../data/scenario_fake_real_source/satire_esp/lexicon_rates_without_stopwords_REAL_POL_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_esp/lexicon_rates_without_stopwords_REAL_ECO_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_esp/lexicon_rates_without_stopwords_REAL_CUL_ESTADAO_FOLHA_SENTENCES_ALL.csv'
]
repetitions = 500
...
train_dataset_path = '../../../data/scenario_fake_real_source/satire_eco/lexicon_rates_without_stopwords_REAL_ECO-FAKE_ESTADAO_FOLHA_SENTENCES_ALL.csv'
test_list_paths = [
    '../../../data/scenario_fake_real_source/satire_eco/lexicon_rates_without_stopwords_REAL_ESP_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_eco/lexicon_rates_without_stopwords_REAL_POL_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_eco/lexicon_rates_without_stopwords_REAL_CUL_ESTADAO_FOLHA_SENTENCES_ALL.csv'
]
repetitions = 500
...
train_dataset_path = '../../../data/scenario_fake_real_source/satire_cul/lexicon_rates_without_stopwords_REAL_CUL-FAKE_ESTADAO_FOLHA_SENTENCES_ALL.csv'
test_list_paths = [
    '../../../data/scenario_fake_real_source/satire_cul/lexicon_rates_without_stopwords_REAL_ESP_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_cul/lexicon_rates_without_stopwords_REAL_ECO_ESTADAO_FOLHA_SENTENCES_ALL.csv',
    '../../../data/scenario_fake_real_source/satire_cul/lexicon_rates_without_stopwords_REAL_POL_ESTADAO_FOLHA_SENTENCES_ALL.csv'
]
repetitions = 500
